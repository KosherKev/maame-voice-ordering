...
class VoiceController {
    ...
    async inboundVoiceWebhook(req, res, next) {
        try {
            ...
            if (!sessionId || !callerNumber || !isActive) {
                throw new index_js_2.ValidationError('Missing required Africa\'s Talking session fields (sessionId, callerNumber, isActive)');
            }
     ...
     */
    async streamTtsAudio(req, res, next) {
        try {
            const { text, lang } = req.query;
            if (!text || typeof text !== 'string') {
                throw new index_js_2.ValidationError('Query parameter "text" is required for TTS streaming');
            }
            const languageCode = lang || 'tw';
            // Call TTS API to synthesize text into audio buffer
            const audioBuffer = await index_js_1.ttsClient.synthesize(text, languageCode);
            res.set({
