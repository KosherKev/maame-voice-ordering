...
class VoiceService {
     ...
     */
    async handleCallStart(sessionId, callerNumber) {
        // Check if session already exists (re-entrant call start)
        let session = await prisma_js_1.prisma.callSession.findUnique({
            where: { id: sessionId },
        });
        if (!session) {
            session = await prisma_js_1.prisma.callSession.create({
                data: {
                    id: sessionId,
     ...
     */
    async processSpeechTurn(sessionId, callerNumber, recordingUrl) {
        // Load session
        const session = await prisma_js_1.prisma.callSession.findUnique({
            where: { id: sessionId },
        });
        if (!session) {
            throw new index_js_2.NotFoundError(`Call session not found: ${sessionId}`);
        }
        // 1. Transcribe speech using ASR (Twi/English)
        const transcription = await index_js_1.asrClient.transcribe(recordingUrl, 'tw');
        ...
        const currentOrder = await prisma_js_1.prisma.order.findUnique({
        ...
        const decision = await index_js_1.llmClient.processSpeech(catalog, basket, updatedTranscript, transcription);
        ...
        if (currentOrder && !currentOrder.llmProviderUsed) {
            await prisma_js_1.prisma.order.update({
                ...
                data: { llmProviderUsed: env_js_1.env.LLM_PROVIDER },
            });
        }
     ...
     */
    async handleBasketUpdate(sessionId, callerNumber, currentOrder, decision, catalog) {
        ...
        for (const item of matchedItems) {
            ...
            const existingItem = order.orderItems?.find((oi) => oi.productId === item.productId)
                || await prisma_js_1.prisma.orderItem.findFirst({
            ...
            if (decision.intent === 'add_item') {
                ...
                if (existingItem) {
                    ...
                    if (newQty <= 0) {
                        await prisma_js_1.prisma.orderItem.delete({
                            where: { id: existingItem.id },
                        });
                    }
                    else {
        ...
        // Recalculate total
        const allItems = await prisma_js_1.prisma.orderItem.findMany({
     ...
     */
    async handleOrderConfirm(sessionId, currentOrder, decision) {
        if (!currentOrder) {
     ...
     */
    async handleOrderCancel(sessionId, orderId) {
        if (orderId) {
            await prisma_js_1.prisma.order.update({
                where: { id: orderId },
     ...
     */
    async appendTranscript(sessionId, speaker, text) {
        const session = await prisma_js_1.prisma.callSession.findUnique({
            where: { id: sessionId },
        });
        if (!session)
