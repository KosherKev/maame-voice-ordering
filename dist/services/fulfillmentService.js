...
class FulfillmentService {
    ...
    async processOrderPaid(orderId) {
        ...
        const existing = await prisma_js_1.prisma.vendorFulfillment.findFirst({
            where: { orderId },
        });
        if (existing) {
        ...
        const vendors = await prisma_js_1.prisma.$queryRaw `
    ...
    `;
        ...
        const products = await prisma_js_1.prisma.$queryRaw `
    ...
    `;
        ...
        await index_js_1.moolreClient.sendSms(vendor.phone, message);
        // Create VendorFulfillment and update Order status atomically
        const subtotal = orderItems.reduce((acc, item) => acc + item.quantity * item.unitPriceInPesewas, 0);
        await prisma_js_1.prisma.$transaction([
            prisma_js_1.prisma.vendorFulfillment.create({
                data: {
                    orderId,
                    vendorId,
     ...
     */
    async getFulfillmentsForOrder(orderId) {
        return prisma_js_1.prisma.vendorFulfillment.findMany({
            where: { orderId },
            include: {
                disbursements: true,
            },
     ...
     */
    async markFulfillmentDelivered(fulfillmentId, idempotencyKey, adminId) {
        const fulfillment = await prisma_js_1.prisma.vendorFulfillment.findUnique({
            where: { id: fulfillmentId },
            include: { order: true },
        });
        if (!fulfillment) {
            throw new index_js_2.NotFoundError(`Fulfillment not found: ${fulfillmentId}`);
        }
        if (fulfillment.deliveryStatus === 'delivered') {
            throw new index_js_2.FulfillmentAlreadyDeliveredError(`Fulfillment already marked delivered: ${fulfillmentId}`);
        }
        const order = fulfillment.order;
        const allowedStatuses = ['paid', 'vendor_notified', 'out_for_delivery'];
        if (!allowedStatuses.includes(order.status)) {
            throw new index_js_2.InvalidStateTransitionError('order not yet paid');
        }
    ...
    `;
        const vendor = vendors[0];
        if (!vendor) {
            throw new index_js_2.NotFoundError(`Vendor not found for fulfillment: ${fulfillment.vendorId}`);
        }
        ...
        this.initiatePayoutTransfer(fulfillmentId, vendor, fulfillment.subtotalInPesewas, idempotencyKey).catch((err) => {
            console.error(`Asynchronous payout failed to initiate for fulfillment ${fulfillmentId}:`, err);
        });
        const updated = await prisma_js_1.prisma.vendorFulfillment.findUnique({
            where: { id: fulfillmentId },
        });
        if (!updated) {
            throw new index_js_2.NotFoundError('Fulfillment not found after update');
        }
        return updated;
    }
    /**
     ...
     */
    async initiatePayoutTransfer(fulfillmentId, vendor, amountInPesewas, externalRef) {
        try {
            const { moolreTransactionId } = await index_js_1.moolreClient.initiateTransfer({
                amountInPesewas,
                vendorPhone: vendor.phone,
                momoChannel: vendor.momoChannel,
                externalRef,
            });
            await prisma_js_1.prisma.disbursement.update({
            ...
            await prisma_js_1.prisma.$transaction([
                prisma_js_1.prisma.disbursement.update({
                    where: { externalref: externalRef },
                    data: { status: 'failed' },
                }),
                prisma_js_1.prisma.vendorFulfillment.update({
                    where: { id: fulfillmentId },
     ...
     */
    static MAX_POLL_COUNT = 48; // ~24h of exponential backoff before giving up
    async pollPendingDisbursements() {
        const now = new Date();
        const pendingDisbursements = await prisma_js_1.prisma.disbursement.findMany({
            where: {
                status: 'pending',
                nextPollAt: { lte: now },
            },
        ...
        for (const disbursement of pendingDisbursements) {
            try {
                const result = await index_js_1.moolreClient.getTransactionStatus(disbursement.externalref);
                if (result.status === 'success') {
                    ...
                    if (pollCount >= FulfillmentService.MAX_POLL_COUNT) {
                        console.error(`🚨 ALERT: Disbursement ${disbursement.id} exceeded max poll count (${FulfillmentService.MAX_POLL_COUNT}). ` +
                        ...
                        console.log(`Disbursement ${disbursement.id} still pending (poll ${pollCount}/${FulfillmentService.MAX_POLL_COUNT}), next check in ${delaySeconds}s`);
                    }
                }
            }
            catch (err) {
...
exports.default = exports.fulfillmentService;
